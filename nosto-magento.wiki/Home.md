...
Nosto also keeps track of the order statuses, i.e. when an order is changed to "paid" or "canceled" the order is updated over an API.<br>
Note that Nosto has a time window of 30 minutes for cancelled orders be taken into account.
...
Magento Enterpise Edition 1.11.x.x - 1.14.x.x