...
| [VariantSelector](./#variantselector) | Category level templating |
...
| Attribute | Description                                                                                                                                              |
| --------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `name`    | **Required.** Name used for analytics and localStorage persistence. The popup's closed state will be remembered.                                         |
...
Simple popup without segment targeting:
...
The popup component uses the following attributes for interaction:

| Attribute  | Description                                                                      |
| ---------- | -------------------------------------------------------------------------------- |
| `n-close`  | Marks an element as a close trigger. Clicking will close the popup permanently. |
| `n-ribbon` | Marks an element as a ribbon trigger. Clicking will minimize popup to ribbon.   |

The popup supports two display modes:
- **Dialog mode**: Full popup displayed as a modal dialog
- **Ribbon mode**: Minimized view showing only the ribbon slot content
...
A custom element that displays a product card using Shopify product data. Fetches product data from `/products/<handle>.js` and renders a card with product image, title, price, and optional brand, discount, and rating information.

The component renders inside a shadow DOM with encapsulated styles. Styling can be customized using CSS custom properties.
...
| Event           | Description                                                      |
| --------------- | ---------------------------------------------------------------- |
| `variantchange` | Emitted when product variant selection changes within the card.  |

#### Markup Attributes

| Attribute | Description                                                         |
| --------- | ------------------------------------------------------------------- |
| `n-atc`   | Marks an element as Add to cart trigger for the selected variant.  |
...
A custom element that displays product variant options as clickable pills. Fetches product data from `/products/<handle>.js` and renders option rows with clickable value pills. Optionally preselects the first value for each option and highlights the currently selected choices.
...
Variant selector with preselection:
...
Variant selector with event handling:

```html
<nosto-variant-selector 
  handle="awesome-product" 
  preselect
  onvariantchange="handleVariantChange(event)">
</nosto-variant-selector>
...
| Event           | Description                                                                       |
| --------------- | --------------------------------------------------------------------------------- |
| `variantchange` | Emitted when variant selection changes, contains `{ variant, product }` in detail |

#### State Management

The component maintains internal state for current selections and provides visual feedback:

- **Active state**: Applied to currently selected options
- **Unavailable state**: Applied to options that have no available variants
- **Disabled state**: Applied to options that are incompatible with current selections
