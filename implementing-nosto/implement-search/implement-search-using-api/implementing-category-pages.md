...
For JS API integrations, there is no built-in fallback functionality for category merchandising. Merchants need to implement their own fallback mechanism to ensure a reliable user experience.

### When to Use Fallback

Your category implementation should fall back to the native category page solution in the following scenarios:

* **API Errors**: If the category query returns an error (network issues, server errors, etc.)
...
* Consider implementing progressive enhancement where Nosto category merchandising enhances the native experience
* Maintain consistency with your search fallback implementation
